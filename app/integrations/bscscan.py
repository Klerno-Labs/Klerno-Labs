...
def fetch_account_tx_bscscan(
    ...
    *,
    ...
    token = (
        _get({"module": "account", "action": "tokentx", **common}, api_key).get(
            "result", []
        )
        or []
    )
    internal = (
        _get({"module": "account", "action": "txlistinternal", **common}, api_key).get(
            "result", []
        )
        or []
    )
...
def bscscan_json_to_transactions(
    ...
    for it in payload.get("normal", []):
        try:
            ...
            direction = (
                "in" if to_addr == acct else ("out" if from_addr == acct else "other")
            )
