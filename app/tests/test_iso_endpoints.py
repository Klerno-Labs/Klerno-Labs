...
def test_validate_xml_endpoint_pain001():
    ...
    r1 = client.post("/enterprise/iso20022/build-message", json=payload, headers=_headers())
    assert r1.status_code == 200
    xml = r1.json().get("xml", "").encode("utf-8")

    r2 = client.post("/enterprise/iso20022/validate-xml", content=xml, headers=_headers())
