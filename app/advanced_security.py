...
class BehavioralAnalyzer:
    ...
    def detect_anomalies(self, user_id: str, current_activity: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if current_endpoint:
            ...
            recent_requests=[
                req for req in self.session_data.get(current_activity.get("session_id", ""), [])
...
class ThreatIntelligence:
    ...
    def check_ip_reputation(self, ip_address: str) -> Dict[str, Any]:
        ...
        with self.lock:
            if ip_address in self.reputation_cache:
                ...
                if cached["timestamp"] > datetime.now(timezone.utc) - timedelta(hours=1):
                    return cached
    ...
    def _categorize_pattern(self, pattern: str) -> str:
        ...
        if any(sql_keyword in pattern.lower() for sql_keyword in ["union", "select", "drop", "insert"]):
            ...
            return "xss"
        elif any(cmd_keyword in pattern.lower() for cmd_keyword in [";", "|", "wget", "curl"]):
        ...
        try:
            ...
            with self.lock:
                ...
                self.reputation_cache={
                    ip: data for ip, data in self.reputation_cache.items()
                    if data.get("timestamp", cutoff_time) > cutoff_time
                }

                logger.info(f"Updated threat feeds: {new_count - old_count} new malicious IPs added")
...
class AdvancedFirewall:
    ...
    def block_ip(self, ip_address: str, duration_minutes: int = 60) -> None:
        ...
        with self.lock:
            ...
            threading.Timer(duration_minutes * 60, self._unblock_ip, args=[ip_address]).start()
    ...
    def check_rate_limit(self, ip_address: str, endpoint: str, limit: int = 100) -> bool:
        ...
        with self.lock:
            ...
            # Clean old requests
            self.rate_limits[ip_address][endpoint] = [
                req_time for req_time in self.rate_limits[ip_address][endpoint]
...
class CryptographicManager:
    ...
    def decrypt_sensitive_data(self, encrypted_data: str, key_id: str = "master") -> str:
    ...
    def verify_signature(self, data: str, signature: str, key_id: str = "master") -> bool:
    ...
    def analyze_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            if analysis_result["threats"]:
                self.security_metrics["threats_detected"] += len(analysis_result["threats"])
...
def decrypt_data(encrypted_data: str, key_id: str = "master") -> str:
    """Decrypt sensitive data."""
    return security_orchestrator.crypto_manager.decrypt_sensitive_data(encrypted_data, key_id)
...
class AdvancedSecurityOrchestrator:
    ...
    async def update_threat_intelligence(self) -> List[Dict[str, Any]]:
        ...
        return [
            {"type": "malicious_ip", "count": len(self.security_orchestrator.threat_intelligence.malicious_ips)},
                {"type": "suspicious_patterns", "count": len(self.security_orchestrator.threat_intelligence.suspicious_patterns)}
        ]
    ...
    async def run_security_assessment(self) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_threats=sum(1 for t in recent_threats if t.get("threat_level") == ThreatLevel.CRITICAL)
            high_threats=sum(1 for t in recent_threats if t.get("threat_level") == ThreatLevel.HIGH)
    ...
    def get_threat_status(self) -> Dict[str, Any]:
        ...
        return {
            "active_threats": len(recent_threats),
                "threat_levels": {
                "critical": sum(1 for t in recent_threats if t.get("threat_level") == ThreatLevel.CRITICAL),
                    "high": sum(1 for t in recent_threats if t.get("threat_level") == ThreatLevel.HIGH),
                    "medium": sum(1 for t in recent_threats if t.get("threat_level") == ThreatLevel.MEDIUM),
                    "low": sum(1 for t in recent_threats if t.get("threat_level") == ThreatLevel.LOW)
            },
                "timestamp": datetime.now(timezone.utc).isoformat()
        }
