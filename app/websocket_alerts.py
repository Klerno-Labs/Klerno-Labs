...
class WebSocketManager:
    ...
    async def send_risk_alert(
        ...
        alert = WebSocketAlert(
            ...
            message=(
                f"Transaction with risk score {risk_score:.2f} "
    ...
    async def _send_to_websocket(self, websocket: WebSocket, data: Dict[str, Any]) -> bool:
    ...
    def _should_receive_alert(self, websocket: WebSocket, alert: WebSocketAlert) -> bool:
    ...
    async def get_connection_stats(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            total_connections = sum(len(websockets) for websockets in self.connections.values())
            unique_users = len(self.connections)
            pending_alerts_count = sum(len(alerts) for alerts in self.pending_alerts.values())
...
async def handle_websocket_connection(websocket: WebSocket, user_id: str, tier: str = "starter"):
