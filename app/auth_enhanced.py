...
from .security_session import verify_pw as verify_password
    ...
    try:
        ...
        if not user:
            return RedirectResponse(
                url="/auth / login?error=Invalid email or password", status_code=302
            )

        # Check if user is blocked
        if user.is_blocked():
            return RedirectResponse(
                url="/auth / login?error=Account is blocked", status_code=302
            )
        ...
        return RedirectResponse(
            url="/auth / login?error=Login error occurred", status_code=302
        )
...
async def api_login(login_data: LoginRequest) -> AuthResponse:
    ...
    try:
        ...
        if user.is_blocked():
            return AuthResponse(success=False, message="Account is blocked")
        ...
        return AuthResponse(success=False, message="Login error occurred")
...
async def signup(request: Request, email: str = Form(...), password: str = Form(...)):
    ...
    try:
        ...
        if admin_manager.get_user_by_email(email):
            return RedirectResponse(
                url="/auth / signup?error=User already exists", status_code=302
            )

        # Create user account (default role: USER)
        result = admin_manager.create_user(
            email=email, password=password, role=UserRole.USER, is_premium=False
        )

        if not result["success"]:
            return RedirectResponse(
                url=f"/auth / signup?error={result['message']}", status_code=302
            )
        ...
        return RedirectResponse(
            url="/auth / signup?error=Signup error occurred", status_code=302
        )
...
async def api_signup(signup_data: SignupRequest) -> AuthResponse:
    ...
    try:
        ...
        return AuthResponse(success=False, message="Signup error occurred")
