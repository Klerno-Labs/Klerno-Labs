...
def notify_if_alert(tagged: TaggedTransaction) -> dict[str, Any]:
    ...
    subject = (
        f"[Klerno Labs Alert] {tagged.category or 'unknown'} — risk {round(tagged.score or 0, 3)}"
    )
    lines = [
...
def report_csv(req: ReportRequest, _auth: bool = Security(enforce_api_key)):
    ...
    mask_wallet = (
        True if not wallets else (df["to_addr"].isin(wallets) | df["from_addr"].isin(wallets))
    )
...
def export_csv_from_db(
    wallet: str | None = None, limit: int = 1000, _auth: bool = Security(enforce_api_key)
):
...
def _check_key_param_or_header(
    key: str | None = None, x_api_key: str | None = Header(default=None)
):
...
_METRICS_CACHE: dict[tuple[float | None, int | None], tuple[float, dict[str, Any]]] = {}
_METRICS_TTL_SEC = 5.0


def _metrics_cached(threshold: float | None, days: int | None) -> dict[str, Any] | None:
    key = (threshold, days)
...
def _metrics_put(threshold: float | None, days: int | None, data: dict[str, Any]):
    key = (threshold, days)
...
@app.get("/metrics")
def metrics(
    threshold: float | None = None, days: int | None = None, _auth: bool = Security(enforce_api_key)
):
    ...
    series = [
        ...
        for d, v in zip(grp["day"], grp["avg_risk"], strict=False)
    ]
...
async def ui_xrpl_fetch_and_save(
    account: str, limit: int = 10, _user=Depends(require_paid_or_admin), _=Depends(csrf_protect_ui)
):
    raw = fetch_account_tx(account, limit=limit)
    txs = xrpl_json_to_transactions(account, raw)
    saved = 0
...
def ui_search_transactions(
    wallet_from: str | None = None,
    wallet_to: str | None = None,
    tx_type: str | None = None,  # 'sale'|'purchase' -> 'out'|'in'; or 'in'|'out'
    date_from: str | None = None,
    date_to: str | None = None,
    min_amount: float | None = None,
    max_amount: float | None = None,
    category: str | None = None,
    risk_bucket: str | None = None,  # low|medium|high
    limit: int = 1000,
    _user=Depends(require_paid_or_admin),
):
    rows = store.list_all(limit=10000)
    ...
    out = pd.DataFrame(
        {
            ...
            "Risk Bucket": df["risk_score"].apply(
...
def notify_test(payload: NotifyRequest = Body(...), _auth: bool = Security(enforce_api_key)):
    return _send_email(
        "Klerno Labs Test", "✅ Your Klerno Labs email system is working!", payload.email
    )
