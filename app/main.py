...
try:
    from .llm import apply_filters as _llm_apply_filters  # optional
    from .llm import (
        ask_to_filters,
        explain_batch,
        explain_selection,
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        self, request: Request, call_next: Callable[[Request], Awaitable[HTMLResponse]]
    ):
...
COOKIE_SECURE_MODE = os.getenv(
    "COOKIE_SECURE", "auto"
).lower()  # "auto" | "true" | "false"
    ...
    try:
        while True:
            ...
            if "watch" in data and isinstance(data["watch"], list):
                watch = {
                    str(x).strip().lower() for x in data["watch"] if isinstance(x, str)
                }
...
def notify_if_alert(tagged: TaggedTransaction) -> Dict[str, Any]:
    ...
    if (tagged.score or 0) < threshold:
        return {
            "sent": False,
            "reason": f"score {tagged.score} < threshold {threshold}",
        }
...
def login_submit(request: Request, email: str = Form(...), password: str = Form(...)):
    ...
    is_paid = (
        bool(user.get("subscription_active"))
...
def signup_submit(request: Request, email: str = Form(...), password: str = Form(...)):
    ...
    user = store.create_user(
        e, hash_pw(password), role=role, subscription_active=sub_active
    )
    token = issue_jwt(user["id"], user["email"], user["role"])
...
def me_settings_post(payload: SettingsPayload, user=Depends(require_user)):
    ...
    patch = {
        k: v for k, v in payload.model_dump(exclude_none=True).items() if k in allowed
    }
...
def analyze_batch(txs: List[Transaction], _auth: bool = Security(enforce_api_key)):
    ...
    for tx in txs:
        ...
        category = tag_category(tx)
        tagged.append(
            TaggedTransaction(**_dump(tx), score=risk, flags=flags, category=category)
        )
    return {
...
def parse_xrpl(
    account: str, payload: List[Dict[str, Any]], _auth: bool = Security(enforce_api_key)
):
...
def get_transactions_for_wallet(
    wallet: str, limit: int = 100, _auth: bool = Security(enforce_api_key)
):
...
async def xrpl_fetch_and_save(
    account: str, limit: int = 10, _auth: bool = Security(enforce_api_key)
):
    raw = fetch_account_tx(account, limit=limit)
    txs = xrpl_json_to_transactions(account, raw)
    saved = 0
    ...
    for tx in txs:
        ...
        tagged = TaggedTransaction(
            **_dump(tx), score=risk, flags=flags, category=category
        )
        d = tagged.model_dump()
        d["risk_score"] = d.get("score")
        d["risk_flags"] = d.get("flags")
...
def export_csv_from_db(
    ...
    rows = (
        store.list_by_wallet(wallet, limit=limit)
        if wallet
...
def _check_key_param_or_header(
    key: Optional[str] = None, x_api_key: Optional[str] = Header(default=None)
):
...
_METRICS_CACHE: Dict[
    Tuple[Optional[float], Optional[int]], Tuple[float, Dict[str, Any]]
] = {}
_METRICS_TTL_SEC = 5.0


def _metrics_cached(
    threshold: Optional[float], days: Optional[int]
...
async def ui_analyze_sample(
    _user=Depends(require_paid_or_admin), _=Depends(csrf_protect_ui)
):
    data_path = os.path.join(BASE_DIR, "..", "data", "sample_transactions.csv")
    df = pd.read_csv(data_path)
...
def ui_recent(
    limit: int = 50, only_alerts: bool = False, _user=Depends(require_paid_or_admin)
):
    if only_alerts:
...
def ui_profile_year(
    year: int, limit: int = 10000, _user=Depends(require_paid_or_admin)
):
    rows = store.list_all(limit=50000)
    yr_rows = []
    for r in rows:
...
def ui_profile_year_export(
    year: int, format: str = "qb", _user=Depends(require_paid_or_admin)
):
    ...
    if not rows:
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=Transactions-{year}.csv"
            },
        )
    df = pd.DataFrame(rows)
    ...
    out = pd.DataFrame(
        {
            "Date": (
                pd.to_datetime(df["timestamp"], errors="coerce").dt.strftime("%Y-%m-%d")
            ),
            "Time": (
                pd.to_datetime(df["timestamp"], errors="coerce").dt.strftime("%H:%M:%S")
            ),
            ...
            "Risk Score": (
                df["risk_score"].apply(lambda x: float(x) if pd.notna(x) else 0.0)
            ),
            "Risk Bucket": (
                df["risk_score"].apply(
                    lambda x: _risk_bucket(x if pd.notna(x) else 0.0)
                )
            ),
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": (
                f"attachment; filename=Transactions-{year}-QuickBooks.csv"
            )
        },
    )
...
def admin_test_email(request: Request):
    ...
    if key != expected:
        return HTMLResponse(
            content="Unauthorized. Append ?key=YOUR_API_KEY", status_code=401
        )
    tx = Transaction(
...
def explain_summary(
    days: int = 7, wallet: str | None = None, _auth: bool = Security(enforce_api_key)
):
    try:
        rows = (
            store.list_by_wallet(wallet, limit=5000)
            if wallet
            else store.list_all(limit=5000)
        )
...
def ai_anomaly_scores(limit: int = 100, _user=Depends(require_paid_or_admin)):
    ...
    if df["amount"].std(ddof=0) == 0:
        ...
        df["anomaly_score"] = (df["amount"] - df["amount"].mean()) / (
            df["amount"].std(ddof=0) or 1
        )
    df["anomaly_score"] = df["anomaly_score"].abs()
    df.sort_values(
        ["anomaly_score", "timestamp"], ascending=[False, False], inplace=True
    )
