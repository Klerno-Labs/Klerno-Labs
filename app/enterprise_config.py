...
class ISO20022Config:
    ...
    supported_message_types: List[str] = field(default_factory=lambda: [
        "pain.001", "pain.002", "camt.053", "camt.054"
    ])
...
class EnterpriseConfig:
    ...
    def from_env(cls) -> 'EnterpriseConfig':
        ...
        if os.getenv("SECURITY_BEHAVIORAL_ANALYSIS"):
            config.security.behavioral_analysis = os.getenv("SECURITY_BEHAVIORAL_ANALYSIS").lower() == "true"
        
        # Performance settings
        if os.getenv("PERFORMANCE_CACHING"):
            config.performance.caching_enabled = os.getenv("PERFORMANCE_CACHING").lower() == "true"
    ...
    def validate(self) -> List[str]:
        ...
        if self.quality.target_overall_score < 95.0:
            issues.append("Overall quality score target is below enterprise standards (95%)")
        ...
        if not self.security.behavioral_analysis and self.security.enabled:
            issues.append("Behavioral analysis should be enabled for enterprise security")
...
__all__ = [
    'EnterpriseConfig', 'ISO20022Config', 'SecurityConfig', 'PerformanceConfig',
    'MonitoringConfig', 'ResilienceConfig', 'TestingConfig', 'QualityConfig',
    'enterprise_config'
]