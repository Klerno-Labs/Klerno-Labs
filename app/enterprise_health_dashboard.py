...
class EnterpriseHealthMonitor:
    ...
    def __init__(self, database_path: str = "./data/klerno.db"):
        ...
        self._health_thread = threading.Thread(target=self._health_check_worker, daemon=True)
    ...
    def _init_database(self):
        ...
        try:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_health_timestamp ON health_check_results(timestamp)")
    ...
    def register_alert_rule(self, 
        ...
        with self._lock:
            self.alert_rules[rule_name] = {
                ...
                "message_template": message_template or f"Alert condition triggered: {rule_name}"
            }
    ...
    def _register_default_alert_rules(self):
        ...
        def high_cpu_condition(metrics):
            recent_metrics = [m for m in metrics if (datetime.now() - m.timestamp).total_seconds() < 300]
        ...
        def service_down_condition(health_results):
            recent_results = [r for r in health_results if (datetime.now() - r.timestamp).total_seconds() < 180]
            service_status = {}
            ...
            for service_name, check_function in self.health_checks.items():
                try:
                    ...
                    if result.status == "healthy":
                        logger.debug(f"[HEALTH] {service_name}: {result.status} ({result.response_time_ms:.1f}ms)")
                    else:
                        logger.warning(f"[HEALTH] {service_name}: {result.status} - {result.error_message}")
                        
                except Exception as e:
                    logger.error(f"[HEALTH] Health check failed for {service_name}: {e}")
    ...
    def _cleanup_old_data(self):
        ...
        try:
            ...
            with self._lock:
                self.health_results = [
                    r for r in self.health_results 
        ...
        with self._lock:
            ...
            if len(recent_metrics) > 1:
                performance_trends = {
                    ...
                    "timestamps": [m.timestamp.isoformat() for m in recent_metrics[-20:]]
                }
            ...
            return {
                ...
                "alert_counts": {
                    ...
                    "error": len([a for a in active_alerts if a.get("severity") == "error"]),
    ...
    def _calculate_overall_status(self) -> str:
        ...
        critical_alerts = [a for a in self.active_alerts if not a.resolved and a.severity == "critical"]
        if critical_alerts:
            return "critical"
        
        # Check for error alerts
        error_alerts = [a for a in self.active_alerts if not a.resolved and a.severity == "error"]
        if error_alerts:
            return "error"
        ...
        for service_name in self.health_checks.keys():
            service_results = [
                ...
                if r.service_name == service_name and 
                   (datetime.now() - r.timestamp).total_seconds() < 300
            ]
            if service_results:
                latest = max(service_results, key=lambda x: x.timestamp)
    ...
    def _calculate_avg_response_time(self) -> float:
        ...
        recent_results = [
            r for r in self.health_results 
            if (datetime.now() - r.timestamp).total_seconds() < 3600 and r.response_time_ms > 0
        ]
