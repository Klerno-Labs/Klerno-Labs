...
class ISO20022MessageBuilder:
    ...
    def create_pain001_message(
        ...
        if payment_instructions:
            ...
            ET.SubElement(dbtr_acct_id, "IBAN").text = payment_instructions[0].debtor_account
        ...
        for instruction in payment_instructions:
            ...
            # Purpose
            ET.SubElement(cdt_trf_tx_inf, "Purp").text = instruction.payment_purpose.value
    ...
    def create_pain002_message(
        ...
        for status in payment_statuses:
            ...
            ET.SubElement(tx_inf_and_sts, "OrgnlInstrId").text = status.original_instruction_id
    ...
    def build_pain001_message(self, payment_data: Dict[str, Any]) -> str:
        ...
        default_msg_id = f"MSG-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"
        message_id = payment_data.get("message_id", default_msg_id)
        creation_datetime = self._to_datetime(
            payment_data.get("creation_datetime", datetime.now(timezone.utc))
        )
        ...
        for instr_data in instructions_data:
            ...
            instruction = ISO20022PaymentInstruction(
                ...
                payment_purpose=PaymentPurpose(instr_data.get("payment_purpose", "OTHR")),
    ...
    def build_pain002_message(self, status_data: Dict[str, Any]) -> str:
        ...
        creation_datetime = self._to_datetime(
            status_data.get("creation_datetime", datetime.now(timezone.utc))
        )
        ...
        for status_data_item in statuses_data:
            ...
            status = ISO20022PaymentStatus(
                ...
                original_instruction_id=status_data_item.get("original_instruction_id", ""),
    ...
    def build_camt053_message(self, statement_data: Dict[str, Any]) -> str:
        ...
        creation_datetime = self._to_datetime(
            statement_data.get("creation_datetime", datetime.now(timezone.utc))
        )
        account_id = statement_data.get("account_id", "")
        transactions = statement_data.get("transactions", [])
        
        return self.create_camt053_message(message_id, creation_datetime, account_id, transactions)
...
class ISO20022Validator:
    ...
    def validate_payment_instruction(self, instruction: ISO20022PaymentInstruction) -> List[str]:
...
class ISO20022Parser:
    ...
    def parse_pain001(self, xml_content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Parse group header
            grp_hdr = root.find('.//ns:GrpHdr', namespace) if namespace else root.find('.//GrpHdr')
            if grp_hdr is not None:
                result["group_header"] = {
                    "message_id": self._get_text(grp_hdr, 'MsgId', namespace),
                    "creation_datetime": self._get_text(grp_hdr, 'CreDtTm', namespace),
                    "number_of_transactions": self._get_text(grp_hdr, 'NbOfTxs', namespace),
                    "control_sum": self._get_text(grp_hdr, 'CtrlSum', namespace)
                }
            
            # Parse payment information
            pmt_inf = root.find('.//ns:PmtInf', namespace) if namespace else root.find('.//PmtInf')
            if pmt_inf is not None:
                ...
                for cdt_trf in pmt_inf.findall(cdt_path, namespace):
                    instruction = self._parse_credit_transfer_instruction(cdt_trf, namespace)
        ...
        try:
            ...
            if org is not None:
                result["original_message"] = {
                    ...
                    "original_message_name": self._get_text(org, 'OrgnlMsgNmId', namespace),
                }
            ...
            for tx in root.findall(tx_path, namespace):
                ...
                sts_rsn = tx.find('.//ns:StsRsnInf' if namespace else './/StsRsnInf', namespace)
                if sts_rsn is not None:
                    # Try nested code and proprietary fields
                    rsn = sts_rsn.find('.//ns:Rsn' if namespace else './/Rsn', namespace)
                    if rsn is not None:
                        cd = rsn.find('ns:Cd', namespace) if namespace else rsn.find('Cd')
                        prtry = rsn.find('ns:Prtry', namespace) if namespace else rsn.find('Prtry')
                        if cd is not None and cd.text:
                            reasons_struct["code"] = cd.text
                        if prtry is not None and prtry.text:
                            reasons_struct["proprietary"] = prtry.text
                    ...
                    for add in sts_rsn.findall(addtl_path, namespace):
                        if add is not None and add.text:
                            additional_info_list.append(add.text)
                ...
                entry = {
                    ...
                    "original_instruction_id": self._get_text(tx, 'OrgnlInstrId', namespace),
    ...
    def parse_camt053(self, xml_content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Statement
            stmt = root.find('.//ns:Stmt', namespace) if namespace else root.find('.//Stmt')
            if stmt is not None:
                result["statement"]["id"] = self._get_text(stmt, 'Id', namespace)
                # Account IBAN
                acct = stmt.find('.//ns:Acct' if namespace else './/Acct', namespace)
                if acct is not None:
                    result["statement"]["account"] = self._get_text(acct, 'IBAN', namespace)

                # Balances
                for bal in stmt.findall('.//ns:Bal' if namespace else './/Bal', namespace):
                    ...
                    if tp is not None:
                        cd = (
                            tp.find('ns:CdOrPrtry/ns:Cd', namespace)
                            if namespace
                            else tp.find('CdOrPrtry/Cd')
                        )
                        pr = (
                            tp.find('ns:CdOrPrtry/ns:Prtry', namespace)
                            if namespace
                            else tp.find('CdOrPrtry/Prtry')
                        )
                        if cd is not None and cd.text:
                            bal_type = cd.text
                        elif pr is not None and pr.text:
                            bal_type = pr.text
                    amt_el = bal.find('ns:Amt', namespace) if namespace else bal.find('Amt')
                    bal_ccy = amt_el.get('Ccy', '') if amt_el is not None else ''
                    bal_amt = amt_el.text if amt_el is not None else ''
                    # Dates can be Dt or DtTm under Bal
                    dt = bal.find('ns:Dt', namespace) if namespace else bal.find('Dt')
                    dttm = bal.find('ns:DtTm', namespace) if namespace else bal.find('DtTm')
                    bal_date = ''
                ...
                # Entries
                for ntry in stmt.findall('.//ns:Ntry' if namespace else './/Ntry', namespace):
                    amt_el = ntry.find('.//ns:Amt' if namespace else './/Amt', namespace)
                    currency = amt_el.get('Ccy', '') if amt_el is not None else ''
                    amount_val = amt_el.text if amt_el is not None else ''
                    # Dates under entry
                    bookg_dt_el = (
                        ntry.find('ns:BookgDt', namespace) if namespace else ntry.find('BookgDt')
                    )
                    bookg = ''
                    ...
                    valdt_el = (
                        ntry.find('ns:ValDt', namespace) if namespace else ntry.find('ValDt')
                    )
                    valdt = ''
...
def create_payment_message(payment_instructions: List[ISO20022PaymentInstruction]) -> str:
...
class ISO20022Manager:
    ...
    def _dict_to_instruction(self, data: Dict[str, Any]) -> ISO20022PaymentInstruction:
        ...
        creditor = ISO20022PartyIdentification(name=creditor_data.get("name", "Creditor"))
        return ISO20022PaymentInstruction(
            ...
            payment_purpose=PaymentPurpose(data.get("payment_purpose", PaymentPurpose.OTHR.value)),
            execution_date=data.get("execution_date")
        )

    def validate_message(self, message_data: Union[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def validate_configuration(self) -> bool:
        ...
        try:
            ...
            message = self.create_payment_instruction(MessageType.PAIN_001, payment_data)
