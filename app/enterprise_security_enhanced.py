...
try:
    ...
    USE_REDIS = False
    rate_limit_store: Dict[str, Dict[str, Any]] = {}
    print("Redis connection failed, using in-memory rate limiting")
...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            # Anti-theft protection
            if not AntiTheftProtection.verify_request_integrity(request):
            ...
            # Rate limiting
            if not AntiTheftProtection.check_rate_limits(client_ip, endpoint_type):
