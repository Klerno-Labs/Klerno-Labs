...
def _set_cookie(resp: RedirectResponse, request: Request, jwt: str):
    from .main import SESSION_COOKIE, _cookie_kwargs
...
async def login_google(request: Request):
    return await oauth.google.authorize_redirect(
        request, request.url_for("auth_google_cb")
    )
...
async def auth_google_cb(request: Request):
    ...
    user = store.get_user_by_email(email) or store.create_user(
        email, password_hash="", role="viewer", subscription_active=True
    )
    jwt = issue_jwt(user["id"], user["email"], user["role"])
    resp = RedirectResponse("/dashboard", status_code=303)
    _set_cookie(resp, request, jwt)
    return resp
...
async def login_ms(request: Request):
    return await oauth.microsoft.authorize_redirect(
        request, request.url_for("auth_ms_cb")
    )
...
async def auth_ms_cb(request: Request):
    ...
    if not email:
        return RedirectResponse("/login?error=microsoft")
    user = store.get_user_by_email(email) or store.create_user(
        email, password_hash="", role="viewer", subscription_active=True
    )
    jwt = issue_jwt(user["id"], user["email"], user["role"])
