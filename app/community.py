...
class CommunityManager:
    ...
    def __init__(self):
        ...
        self.votes: Dict[str, Dict[int, VoteType]] = {}  # user_id -> {post_id: vote_type}
...
This has worked well for us and reduces alert fatigue significantly.""",
    ...
    def get_featured_content(self) -> Dict[str, List[Any]]:
        ...
        featured_articles = [
            article for article in self.knowledge_articles.values() 
            if article.is_featured
        ]
        
        popular_posts = sorted(
            [post for post in self.posts.values() if post.status == PostStatus.PUBLISHED],
            ...
            for post in self.posts.values():
                if (post.status == PostStatus.PUBLISHED and
                    (query_lower in post.title.lower() or 
                     ...
                     any(query_lower in tag for tag in post.tags))):
                    results.append({"type": "post", "item": post})
        
        return results
    
    def get_user_contributions(self, user_id: int) -> Optional[UserContribution]:
        """Get user's community contributions and reputation"""
        return self.user_contributions.get(user_id)
    
    def create_post(self, title: str, content: str, post_type: PostType, 
                   author_id: int, author_name: str, tags: List[str] = None) -> CommunityPost:
    ...
    def add_reply(self, post_id: int, content: str, author_id: int, 
                  author_name: str, parent_reply_id: Optional[int] = None) -> CommunityReply:
        """Add a reply to a post"""
        if post_id not in self.posts:
            raise ValueError("Post not found")
        
        reply_id = max(
            [r.id for replies in self.replies.values() for r in replies], 
            default=0
        ) + 1
...
class CollaborationFeatures:
    ...
    def create_shared_workspace(self, name: str, description: str, 
                              owner_id: int, team_members: List[int]) -> Dict[str, Any]:
    ...
    def add_annotation(self, transaction_id: str, user_id: int, 
                      annotation: str, annotation_type: str = "note") -> Dict[str, Any]:
    ...
    def get_team_insights(self, workspace_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "team_stats": {
                "total_annotations": sum(len(annotations) for annotations in self.team_annotations.values()),
                "resolved_issues": 45,
