...
class Settings(BaseSettings):
    ...
    # App configuration
    APP_ENV: str = Field("dev", description="Application environment: 'dev', 'test', or 'production'")
    DEBUG: bool = Field(False, description="Debug mode")
    SECRET_KEY: str = Field(..., description="Secret key for session encryption and JWT")
    ...
    DATABASE_URL: Optional[PostgresDsn] = Field(
        None, description="PostgreSQL connection string (used when USE_SQLITE=false)"
    )
    SQLITE_PATH: str = Field(
        "./data/klerno.db", description="Path to SQLite database (used when USE_SQLITE=true)"
    )
    ...
    SUB_PRICE_XRP: float = Field(
        10.0, description="Price in XRP for a subscription"
    )
    ...
    ALERT_EMAIL_FROM: Optional[str] = Field(None, description="Email to send alerts from")
    ALERT_EMAIL_TO: Optional[str] = Field(None, description="Email to send alerts to")
    ...
    def validate_app_env(cls, v: str) -> str:
        ...
        if v.lower() not in {"dev", "test", "production"}:
