...
async def require_admin_access(current_user: User = Depends(get_current_user)):
    if not current_user.is_manager_or_higher():
        raise HTTPException(status_code=403, detail="Admin access required")
    return current_user

async def require_admin_edit_access(current_user: User = Depends(get_current_user)):
    if not current_user.is_admin_or_higher():
        raise HTTPException(status_code=403, detail="Admin edit access required")
    return current_user

async def require_owner_access(current_user: User = Depends(get_current_user)):
