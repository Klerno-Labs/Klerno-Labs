...
async def require_admin_access(current_user: User | None = Depends(get_current_user)):
    if not current_user or not current_user.is_manager_or_higher():
        raise HTTPException(status_code=403, detail="Admin access required")
    return current_user


async def require_admin_edit_access(
...
async def require_owner_access(current_user: User | None = Depends(get_current_user)):
...
async def admin_dashboard(
    request: Request, current_user: User = Depends(require_admin_access)
):
...
async def get_dashboard_data(
    hours: int = 24, current_user: User = Depends(require_admin_access)
):
...
async def update_user_role(
    ...
    result = admin_manager.update_user_role(
        admin_email=current_user.email, target_email=target_email, new_role=new_role
    )
...
async def block_user(
    ...
    result = admin_manager.block_user(
        admin_email=current_user.email, request=block_request
    )
...
async def unblock_user(
    ...
    result = admin_manager.unblock_user(
        admin_email=current_user.email, target_email=target_email, reason=reason
    )
...
async def get_admin_actions(
    days: int = 30, current_user: User = Depends(require_admin_access)
):
...
async def get_monthly_block_report(
    year: int, month: int, current_user: User = Depends(require_admin_access)
):
...
async def update_notification_email(
    new_email: str = Form(...), current_user: User = Depends(require_owner_access)
):
...
async def get_system_health(current_user: User = Depends(require_admin_access)):
    ...
    if security_metrics and security_metrics.threat_level in ["MEDIUM", "HIGH"]:
        health_status = (
            "warning" if security_metrics.threat_level == "MEDIUM" else "critical"
        )
...
async def test_block_confirmation(
    current_user: User = Depends(require_admin_edit_access),
):
...
async def get_live_status(current_user: User = Depends(require_admin_access)):
    ...
    return {
        ...
        "metrics": {k: v.__dict__ if v else None for k, v in current_metrics.items()},
    }
