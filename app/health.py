...
class HealthChecker:
    ...
    async def check_external_apis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                start_time=time.time()
                async with session.get('https://s2.ripple.com:51234', timeout=5) as response:
                    response_time=(time.time() - start_time) * 1000
                    checks["xrpl"] = {
    ...
    async def comprehensive_health_check(self) -> HealthStatus:
        ...
        checks={
            "database": database_check if not isinstance(database_check, Exception) else {"status": "error", "message": str(database_check)},
                "cache": cache_check if not isinstance(cache_check, Exception) else {"status": "error", "message": str(cache_check)},
                "external": external_checks if not isinstance(external_checks, Exception) else {"status": "error", "message": str(external_checks)},
            ...
            app_metrics={
                "uptime_seconds": round(time.time() - self.start_time, 2),
                    ...
                    "rss_mb": round(psutil.Process().memory_info().rss / 1024 / 1024, 2),
                        "vms_mb": round(psutil.Process().memory_info().vms / 1024 / 1024, 2)
                },
