...
class MetricsCollector:
    ...
    def record_metric(self, metric: Metric) -> None:
        ...
        with self.lock:
            ...
            if metric.metric_type in [MetricType.TIMER, MetricType.COUNTER]:
                self.logger.debug(f"Metric recorded: {metric.name}={metric.value} {metric.unit or ''}")
    ...
    def set_gauge(self, name: str, value: Union[int, float], tags: Dict[str, str] = None, unit: str = None) -> None:
        """Set a gauge metric."""
        metric=Metric(
            name=name,
                value=value,
    ...
    def get_aggregate_stats(self, metric_name: str, metric_type: MetricType) -> Dict[str, Any]:
...
class HealthChecker:
    ...
    async def run_all_checks(self) -> Dict[str, HealthCheck]:
        ...
        return {check.name: check for check in results if isinstance(check, HealthCheck)}
...
class SystemMonitor:
    ...
    def _collect_system_metrics(self) -> None:
        ...
        try:
            ...
            self.metrics.set_gauge("system.disk.usage", (disk.used / disk.total) * 100, unit="%")
            ...
            self.metrics.set_gauge("process.memory.usage", process.memory_percent(), unit="%")
...
def timer_context(metrics_collector: MetricsCollector, metric_name: str, tags: Dict[str, str] = None):
...
class QualityController:
    ...
    def calculate_quality_score(self, metrics: List[Metric]) -> float:
        ...
        if response_times:
            ...
            response_score=max(0, 100 - (p95_response / self.quality_thresholds["response_time_p95"]) * 100)
        ...
        if requests > 0:
            ...
            error_score=max(0, 100 - (error_rate / self.quality_thresholds["error_rate"]) * 100)
        ...
        if cpu_metrics:
            ...
            cpu_score=max(0, 100 - (avg_cpu / self.quality_thresholds["cpu_usage"]) * 100)
            scores.append(cpu_score)

        memory_metrics=[m.value for m in metrics if m.name == "system.memory.usage"]
        if memory_metrics:
            avg_memory=statistics.mean(memory_metrics)
            memory_score=max(0, 100 - (avg_memory / self.quality_thresholds["memory_usage"]) * 100)
...
def get_monitoring_dashboard() -> Dict[str, Any]:
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
            ...
            "system_stats": {
            "cpu_usage": metrics_collector.get_aggregate_stats("system.cpu.usage", MetricType.GAUGE),
                "memory_usage": metrics_collector.get_aggregate_stats("system.memory.usage", MetricType.GAUGE),
                "response_times": metrics_collector.get_aggregate_stats("api.response_time", MetricType.TIMER)
        },
            "recommendations": quality_controller.get_quality_recommendations(recent_metrics)
    }
        ...
        try:
            ...
            return {
                "timestamp": datetime.now(timezone.utc).isoformat(),
                    ...
                    "system_metrics": {
                    "cpu_usage": self.metrics_collector.get_aggregate_stats("system.cpu.usage", MetricType.GAUGE),
                        "memory_usage": self.metrics_collector.get_aggregate_stats("system.memory.usage", MetricType.GAUGE),
                        "response_time": self.metrics_collector.get_aggregate_stats("api.response_time", MetricType.TIMER)
                },
