...
class AdvancedAIRiskEngine:
    ...
    def analyze_transaction(
        ...
        risk_factors=self._analyze_risk_factors(features, transaction_data, user_history)
    ...
    def _extract_features(
        ...
        # Time - based features
        tx_time=transaction.get('timestamp', datetime.now(timezone.utc))
    ...
    def _generate_ai_insights(
        ...
        if is_anomaly:
            insights.append("🤖 AI detected this transaction as anomalous compared to normal patterns")

        if factors.transaction_frequency > 0.7:
            insights.append("🔄 High transaction frequency detected - possible automated trading")

        if factors.amount_anomaly > 0.8:
            insights.append("💰 Unusually large transaction amount compared to typical patterns")

        if factors.time_pattern > 0.5:
            insights.append("⏰ Transaction occurred during unusual hours - potential suspicious activity")

        if factors.geographical_risk > 0.6:
            insights.append("🌍 Geographic risk factors detected - location - based risk assessment triggered")

        if factors.behavioral_pattern > 0.7:
            insights.append("🧠 Behavioral analysis indicates deviation from normal user patterns")

        # Advanced ML insights
        if anomaly_score < -0.5:
            insights.append("📊 Machine learning models flag this as highly unusual transaction")

        return insights


    def _generate_recommendations(self, risk_level: str, factors: RiskFactors) -> List[str]:
        ...
        if factors.geographical_risk > 0.5:
            recommendations.append("🔍 Enhanced due diligence for high - risk jurisdictions")
    ...
    def _is_recent(self, transaction: Dict[str, Any], days: int = 1) -> bool:
        ...
        return (datetime.now(timezone.utc) - tx_time).days <= days
