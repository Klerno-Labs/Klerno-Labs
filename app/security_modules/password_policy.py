...
try:
    ...
    class PasswordHasher:
        ...
        def hash(self, password): return hashlib.pbkdf2_hmac('sha256', password.encode(), b'salt', 100000).hex()
        def verify(self, hash_val, password): return hash_val == self.hash(password)
...
class PasswordSecurityPolicy:
    ...
    def validate(self, password: str, username: Optional[str] = None) -> Tuple[bool, List[str]]:
        ...
        if self.config.require_symbols and not re.search(r'[!@  #$%^&*(),.?":{}|<>]', password):
        ...
        if self.config.check_breaches and len(errors) == 0:
            try:
                if self.check_breached(password):
                    errors.append("This password has been found in data breaches and cannot be used")
