...
class AdvancedSecurityHardening:
    ...
    def _init_security_database(self) -> None:
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_security_events_ip ON security_events(source_ip)")
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_failed_auth_ip ON failed_auth_attempts(ip_address)")
    ...
    def _load_threat_intelligence(self) -> None:
        ...
        try:
            ...
            logger.info(f"✅ Loaded {len(self.threat_intel)} threat indicators and {len(self.blocked_ips)} blocked IPs")
    ...
    def block_ip(self, ip_address: str, reason: str, duration: Optional[timedelta] = None) -> None:
        """Block an IP address."""
    ...
    def detect_suspicious_patterns(self, request_data: Dict[str, Any]) -> List[str]:
        ...
        for key, value in query_params.items():
            for pattern in self.suspicious_patterns:
                if re.search(pattern, str(value), re.IGNORECASE):
                    suspicious_findings.append(f"Suspicious pattern in parameter {key}: {pattern}")
        ...
        for header, value in headers.items():
            if header.lower() in ['user-agent', 'referer', 'x-forwarded-for']:
                for pattern in self.suspicious_patterns:
                    if re.search(pattern, str(value), re.IGNORECASE):
                        suspicious_findings.append(f"Suspicious pattern in header {header}: {pattern}")
        
        # Check for common attack indicators
        combined_data = f"{path} {json.dumps(query_params)} {json.dumps(headers)}"
        
        # SQL injection indicators
        sql_keywords = ['union', 'select', 'insert', 'delete', 'drop', 'alter', 'create', 'exec']
    ...
    def analyze_user_agent(self, user_agent: str) -> Dict[str, Any]:
        ...
        bot_patterns = [
            ...
            'python-requests', 'go-http-client', 'http_request',
    ...
    def log_security_event(self, event_type: str, severity: str, source_ip: str, 
                          ...
                          details: Dict[str, Any] = None, blocked: bool = False) -> None:
    ...
    def log_failed_auth_attempt(self, ip_address: str, username: str = None, 
                               endpoint: str = '', user_agent: str = '') -> bool:
    ...
    def analyze_request_security(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if analysis['risk_score'] >= 70:
            ...
            if analysis['risk_score'] >= 90:
                self.block_ip(ip_address, f"Automated block: risk score {analysis['risk_score']}")
                analysis['blocked'] = True
    ...
    def get_security_dashboard_data(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            top_blocked_ips = [{'ip': row[0], 'events': row[1]} for row in cursor.fetchall()]
