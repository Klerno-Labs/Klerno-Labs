...
class SecurityManager:
    ...
    def is_brute_force(self, identifier: str, window_minutes: int = 15, max_attempts: int = 5) -> bool:
    ...
    def is_rate_limited(self, identifier: str, window_minutes: int = 1, max_requests: int = 60) -> bool:
        """Check rate limiting"""
        now = datetime.now(timezone.utc)
        cutoff = now - timedelta(minutes=window_minutes)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if request.method in ["POST", "PUT", "PATCH"]:
            try:
                ...
                if body and security_manager.check_suspicious_input(body.decode("utf-8", errors="ignore")):
                    security_manager.log_security_event(
...
class AdminAccessLogger:
    ...
    @staticmethod
    def log_admin_action(user_email: str, action: str, details: Dict, request: Request = None):
