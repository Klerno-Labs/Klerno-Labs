...
class FastAPIProfiler:
        ...
        def user_session(user_id: int):
            ...
            for i in range(requests_per_user):
                ...
                try:
                    ...
                    request_end = time.perf_counter()
                    session_times.append((request_end - request_start) * 1000)
                    session_errors.append(f"User {user_id}, Request {i}: {str(e)}")
    ...
    def _generate_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for endpoint, profile in results["endpoint_profiles"].items():
            if "response_stats" in profile and profile["response_stats"]:
...
def main():
    ...
    print(f"Overall Performance: {summary['overall_performance_tier']}")
    ...
    if summary["best_performing_endpoint"]:
        best = summary["best_performing_endpoint"]
        print(f"Best Endpoint: {best['endpoint']} ({best['avg_ms']:.2f}ms)")
    ...
    print(f"⏱️  Total analysis time: {results['total_analysis_time_s']:.2f}s")
