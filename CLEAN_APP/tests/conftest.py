...
# Ensure legacy token helpers are available globally for some older tests that
# call create_access_token()/verify_token() without importing them.
try:
    import builtins

    # Import the stable shim which will delegate to `app.auth` when possible
    # but also provides safe fallbacks. This avoids import-time failures when
